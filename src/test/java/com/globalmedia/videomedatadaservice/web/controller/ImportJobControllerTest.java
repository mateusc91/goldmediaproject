...
import com.globalmedia.videomedatadaservice.controller.ImportJobController;
import com.globalmedia.videomedatadaservice.domain.model.ImportJob;
import com.globalmedia.videomedatadaservice.service.ImportJobService;
...
class ImportJobControllerTest {
    ...
    void getAllImportJobs_ReturnsPagedResponse() throws Exception {
        ...
        when(importJobService.getPaginatedImportJobs(eq(null), eq(null), eq(0), eq(10), eq("createdAt"), eq("DESC"))).thenReturn(PagedResponse.from(page, "createdAt", "DESC"));
    ...
    void getAllImportJobs_WithStatusFilter_ReturnsFilteredPagedResponse() throws Exception {
        List<ImportJobDto> jobs = Arrays.asList(testImportJobDto);
        Page<ImportJobDto> page = new PageImpl<>(jobs, PageRequest.of(0, 10), 1);
        when(importJobService.getPaginatedImportJobs(eq(ImportJob.ImportStatus.COMPLETED), eq(null), eq(0), eq(10), eq("createdAt"), eq("DESC"))).thenReturn(PagedResponse.from(page, "createdAt", "DESC"));
    ...
    void getAllImportJobs_WithSourceFilter_ReturnsFilteredPagedResponse() throws Exception {
        List<ImportJobDto> jobs = Arrays.asList(testImportJobDto);
        Page<ImportJobDto> page = new PageImpl<>(jobs, PageRequest.of(0, 10), 1);
        when(importJobService.getPaginatedImportJobs(eq(null), eq("YouTube"), eq(0), eq(10), eq("createdAt"), eq("DESC"))).thenReturn(PagedResponse.from(page, "createdAt", "DESC"));
    ...
    void getAllImportJobs_WithCustomPaginationAndSorting_ReturnsCustomPagedResponse() throws Exception {
        ...
        when(importJobService.getPaginatedImportJobs(eq(null), eq(null), eq(1), eq(5), eq("status"), eq("ASC"))).thenReturn(PagedResponse.from(page, "status", "ASC"));
